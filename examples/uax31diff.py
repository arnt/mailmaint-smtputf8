...
def uax31(path):
    ...
    for line in open(path):
        ...
        if len(fields) < 2 or fields[1] not in ('XID_Start', 'XID_Continue'):
            continue
        first, _, last = fields[0].partition('..')
        cps = range(int(first, 16), int(last or first, 16) + 1)
        (start if fields[1] == 'XID_Start' else cont).update(cps)
    return start, cont

def identifierclass_rejects(cp):
    """RFC8264 4.2.3 disallows these regardless of the category rules,
    so a class-by-class reading of rule 2 lets them back in."""
    if cp in F_DISALLOWED:
        return 'F exception, DISALLOWED'
    if cp in OLD_HANGUL_JAMO:
        return 'OldHangulJamo (I)'
    if cp > 0x7E and unicodedata.normalize('NFKC', chr(cp)) != chr(cp):
        return 'HasCompat (Q)'
    return None
...
xid_start, xid_cont = uax31(sys.argv[1] if len(sys.argv) > 1
                            else 'DerivedCoreProperties.txt')
...
rejected = {cp: identifierclass_rejects(cp) for cp in sorted(draft)}
